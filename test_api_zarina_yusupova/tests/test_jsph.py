...
def test_receive_objects(get_objects_endpoint):
    ...
    get_objects_endpoint.check_that_len_of_response_is_correct()


@pytest.mark.parametrize('data', TEST_DATA)
def test_add_object(create_object_endpoint, data):
    create_object_endpoint.create_new_object(payload=data)
    create_object_endpoint.check_that_name_and_data_of_object_are_correct(data)
...
def test_patch_object(patch_object_endpoint, object_id_f):
    payload = {"name": "Fifth object"}
    patch_object_endpoint.make_changes_in_object(object_id_f, payload)
    patch_object_endpoint.check_that_data_of_object_is_correct(TEST_DATA[1])
